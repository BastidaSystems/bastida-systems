...
const uiTranslations = {
  en: {
    navServices: "Services", navMenus: "Menus", navQuote: "Quote", navOperations: "Operations", navSeo: "SEO",
    requestQuote: "Request quote", heroTitle: "Las Vegas catering built to sell, book, cook, and scale.",
    heroCopy: "Cater Vegas combines a premium public website with a complete operations platform for event sales, kitchen production, delivery, staffing, profitability, and Beoflow integration.",
    heroPrimary: "Calculate an instant quote", heroSecondary: "View operations system",
    metricServices: "catering services", metricResponse: "lead response target", metricBeoflow: "Beoflow-ready hub",
    ...
    authorityTitle: "The authority layer for Las Vegas catering searches.", authorityCopy: "Every service vertical gets a conversion-focused page structure, local SEO hooks, and operational handoff into the back office.",
    ...
    quoteTitle: "Instant quote calculator connected to event intake.", quoteCopy: "Guests, event type, timing, and service model generate a live estimate, deposit, Beoflow production forecast, and lead priority.",
    ...
    estimatedTotal: "Estimated total", depositDueLabel: "Deposit due", estimatedMargin: "Estimated margin", beoflowSyncTitle: "Beoflow event sync",
    portalTitle: "Customer portal, ordering, reviews, and SEO content engine.", portalCopy: "The public side moves from discovery to quote to ordering while building local search authority.",
    ...
    reviewsTitle: "Reviews", reviewsCopy: "Testimonials are tagged by event type to support trust and SEO-rich service pages.",
    ...
    confirmedAutomation: "Confirmed event automation", confirmedAutomationCopy: "When an event is confirmed, Cater Vegas sends menu, guest count, date, service style, staffing needs, and rental requirements into Beoflow.",
    autoRecipes: "Generate production recipes", autoInventory: "Deduct inventory", autoPurchasing: "Create purchasing requirements", autoCosts: "Calculate food and labor cost", autoMargin: "Estimate profit margin", autoPrep: "Create prep lists and kitchen schedules",
    runSync: "Run Beoflow sync demo", syncReady: "Ready to sync next confirmed event.", seoTitle: "SEO architecture for Las Vegas catering dominance.", seoCopy: "Structured pages, schema, local content, and Google Business Profile workflows build authority beyond a single landing page.",
    servicePages: "Service pages", servicePagesCopy: "Corporate, wedding, convention, hotel, private event, and holiday catering pages with matching quote forms.",
    locationPages: "Location pages", locationPagesCopy: "Las Vegas Strip, Downtown Las Vegas, Summerlin, Henderson, North Las Vegas, and convention center landing pages.",
    schemaLocal: "Schema and local SEO", schemaLocalCopy: "LocalBusiness, Service, FAQ, Review, BlogPosting, menu schema, and Google Business Profile review capture.",
    footerCopy: "Premium catering website, operations platform, kitchen production system, and Beoflow integration hub.", backTop: "Back to top"
  },
  es: {
    navServices: "Servicios", navMenus: "Menús", navQuote: "Cotización", navOperations: "Operaciones", navSeo: "SEO",
    requestQuote: "Solicitar cotización", heroTitle: "Catering en Las Vegas diseñado para vender, reservar, cocinar y escalar.",
    heroCopy: "Cater Vegas combina un website público premium con una plataforma completa para ventas de eventos, producción de cocina, entregas, staff, rentabilidad e integración con Beoflow.",
    heroPrimary: "Calcular cotización", heroSecondary: "Ver sistema operativo",
    metricServices: "servicios de catering", metricResponse: "respuesta a leads", metricBeoflow: "hub listo para Beoflow",
    ...
    authorityTitle: "La capa de autoridad para búsquedas de catering en Las Vegas.", authorityCopy: "Cada servicio tiene estructura para convertir, señales de SEO local y traspaso operativo al back office.",
    ...
    quoteTitle: "Cotizador instantáneo conectado al intake del evento.", quoteCopy: "Invitados, tipo de evento, horario y modelo de servicio generan estimado, depósito, forecast Beoflow y prioridad de lead.",
    ...
    estimatedTotal: "Total estimado", depositDueLabel: "Depósito", estimatedMargin: "Margen estimado", beoflowSyncTitle: "Sincronización Beoflow",
    portalTitle: "Portal de cliente, pedidos, reseñas y motor SEO.", portalCopy: "La parte pública lleva al cliente desde descubrimiento hasta cotización y pedidos mientras construye autoridad local.",
    ...
    reviewsTitle: "Reseñas", reviewsCopy: "Los testimonios se etiquetan por tipo de evento para apoyar confianza y páginas SEO.",
    ...
    confirmedAutomation: "Automatización de evento confirmado", confirmedAutomationCopy: "Cuando se confirma un evento, Cater Vegas envía menú, invitados, fecha, servicio, staff y rentas a Beoflow.",
    autoRecipes: "Generar recetas de producción", autoInventory: "Descontar inventario", autoPurchasing: "Crear compras necesarias", autoCosts: "Calcular comida y labor", autoMargin: "Estimar margen", autoPrep: "Crear prep lists y horarios de cocina",
    runSync: "Correr demo Beoflow", syncReady: "Listo para sincronizar el próximo evento confirmado.", seoTitle: "Arquitectura SEO para dominar catering en Las Vegas.", seoCopy: "Páginas estructuradas, schema, contenido local y Google Business Profile construyen autoridad más allá de una landing.",
    servicePages: "Páginas de servicio", servicePagesCopy: "Páginas para corporativo, bodas, convenciones, hoteles, privados y fiestas con formularios de cotización.",
    locationPages: "Páginas por ubicación", locationPagesCopy: "Las Vegas Strip, Downtown Las Vegas, Summerlin, Henderson, North Las Vegas y Convention Center.",
    schemaLocal: "Schema y SEO local", schemaLocalCopy: "LocalBusiness, Service, FAQ, Review, BlogPosting, schema de menú y captura de reseñas de Google Business Profile.",
    footerCopy: "Website premium de catering, plataforma operativa, sistema de producción de cocina e integración Beoflow.", backTop: "Volver arriba"
  }
};
...
function localizedService(service) {
  ...
  return { ...service, name: translated[0], copy: translated[1], keyword: translated[2] };
}
...
function calculateQuote() {
  ...
  const foodCost = total * 0.33;
  const laborCost = total * 0.19;
  const recipes = Math.max(4, Math.ceil(guests / 14));
  const purchasing = Math.max(6, Math.ceil(guests / 9));
  const staffHours = Math.max(10, Math.ceil(guests * (serviceLevel === "delivery" ? 0.12 : 0.26)));
  ...
  document.getElementById("beoflowForecast").textContent = currentLanguage === "es"
    ? `Crea ${recipes} recetas de producción, forecast de comida de ${formatMoney(foodCost)}, ${staffHours} horas de staff, forecast laboral de ${formatMoney(laborCost)} y ${purchasing} compras necesarias.`
    : `Creates ${recipes} production recipes, ${formatMoney(foodCost)} food cost forecast, ${staffHours} staff hours, ${formatMoney(laborCost)} labor cost forecast, and ${purchasing} purchasing requirements.`;
}
...
function initInteractions() {
  ...
  document.getElementById("syncBeoflow").addEventListener("click", () => {
    const syncStatus = document.getElementById("syncStatus");
    syncStatus.textContent = currentLanguage === "es"
      ? "Sincronizando evento confirmado con Beoflow..."
      : "Syncing confirmed event with Beoflow...";
    setTimeout(() => {
      syncStatus.textContent = currentLanguage === "es"
        ? "Sincronización Beoflow completa: recetas, inventario, compras, labor, prep lists y margen generados."
        : "Beoflow sync complete: recipes, inventory, purchasing, labor, prep lists, and margin report generated.";
    }, 900);
  });
}
